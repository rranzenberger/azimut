    ...
    <>
      ...
      <main className="relative py-16 md:py-20">
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          {/* Hero — PREMIUM: tema claro com gradiente elegante cream/beige */}
                    ...
                    <div className="pl-0 md:pl-14 space-y-4">
                      ...
                      {section.flags && (
                        <div className={`text-center p-4 rounded-xl border ${
                          theme === 'light' 
                            ? 'bg-gradient-to-br from-white via-[#faf8f5] to-[#f5f0e8] border-[#d4cdc0]/50' 
                            : 'bg-gradient-to-b from-white/5 to-transparent dark:from-white/[0.06] border-azimut-red/20'
                        }`}>
                          <p className={`text-base md:text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-[#1e1c1a]'}`}>{section.flags}</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {section.type === 'grid' && section.items && (
                    <div className="pl-0 md:pl-14 grid sm:grid-cols-2 gap-4 md:gap-6">
                      {section.items.map((item, j) => (
                        <div key={j} className={`p-5 md:p-6 rounded-xl border transition-all ${
                          theme === 'light' 
                            ? 'bg-gradient-to-br from-white via-[#faf8f5] to-[#f5f0e8] border-[#d4cdc0]/50 hover:border-[#c9a96e]/60 hover:shadow-md' 
                            : 'bg-gradient-to-b from-white/5 to-transparent dark:from-white/[0.06] border border-azimut-red/20 hover:border-azimut-red/30'
                        }`}>
                          <div className="flex items-center gap-2 mb-4">
                            <span className="text-2xl">{item.icon}</span>
                            <h4 className={`text-base md:text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-[#1e1c1a]'}`}>{item.title}</h4>
                          </div>
                          <ul className="space-y-2">
                            {item.list.map((li, k) => (
                              <li key={k} className={`flex items-start gap-2 text-sm ${
                                ...
                                <span className={`mt-0.5 flex-shrink-0 ${theme === 'light' ? 'text-azimut-red' : 'text-azimut-red'}`}>•</span>
                                <span>{li}</span>
                              </li>
                            ))}
                          </ul>
                  ...
                  {section.type === 'cards' && section.cards && (
                    <div className="pl-0 md:pl-14 grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                      {section.cards.map((card, j) => (
                        <div key={j} className={`p-4 rounded-xl border transition-all text-center ${
                          theme === 'light' 
                            ? 'bg-gradient-to-br from-white via-[#faf8f5] to-[#f5f0e8] border-[#d4cdc0]/50 hover:border-[#c9a96e]/60 hover:shadow-md' 
                            : 'bg-gradient-to-b from-white/5 to-transparent dark:from-white/[0.06] border border-azimut-red/20 hover:border-azimut-red/30'
                        }`}>
                  ...
                  {section.type === 'contact' && section.contact && (
                    <div className="pl-0 md:pl-14">
                      <div className={`p-6 rounded-2xl border ${
                        theme === 'light' 
                          ? 'bg-gradient-to-br from-[#faf8f5] via-[#f5f0e8] to-[#ebe5db] border-[#d4cdc0]/60 shadow-md' 
                          : 'bg-gradient-to-br from-azimut-red/10 to-azimut-red/5 border-azimut-red/30'
                      }`}>
                        <div className="grid sm:grid-cols-2 gap-4 mb-4">
                          <div>
                            <p className={`text-sm mb-1 ${theme === 'light' ? 'text-[#7a756e]' : 'text-theme-text-secondary'}`}>📧 {lang === 'fr' ? 'Courriel Général' : lang === 'es' ? 'Email General' : lang === 'en' ? 'General Email' : 'Email Geral'}</p>
                            <a 
                              href={`mailto:${section.contact.email}`}
                              className={`text-lg font-bold ${theme === 'dark' ? 'text-white hover:text-azimut-red' : 'text-[#1e1c1a] hover:text-azimut-red'} transition-colors`}
                            >
                              {section.contact.email}
                            </a>
                          </div>
                          <div>
                            <p className={`text-sm mb-1 ${theme === 'light' ? 'text-[#7a756e]' : 'text-theme-text-secondary'}`}>⚖️ {lang === 'fr' ? 'Questions Juridiques' : lang === 'es' ? 'Consultas Legales' : lang === 'en' ? 'Legal Inquiries' : 'Questões Jurídicas'}</p>
                            <a 
                        ...
                        <div className={`flex flex-wrap gap-4 pt-4 border-t ${theme === 'light' ? 'border-[#d4cdc0]/60' : 'border-azimut-red/20'}`}>
                          {section.contact.locations.map((loc, j) => (
                            <span key={j} className={`text-sm ${theme === 'light' ? 'text-[#5a554e]' : 'text-theme-text-secondary'}`}>
                              {loc}
                            </span>
                          ))}
                        </div>
          ...
          {/* Footer */}
          <footer className={`mt-12 pt-8 text-center ${theme === 'light' ? 'border-t border-[#d4cdc0]/60' : 'border-t border-azimut-red/20'}`}>
            <p className={`text-sm ${theme === 'light' ? 'text-[#7a756e]' : 'text-theme-text-secondary/70'}`}>
