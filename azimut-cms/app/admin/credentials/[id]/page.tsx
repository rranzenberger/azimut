import { cookies } from 'next/headers';
import { redirect } from 'next/navigation';
import { verifyAuthToken } from '@/src/lib/auth';
import { prisma } from '@/src/lib/prisma';
import { CredentialEditForm } from '../components/CredentialEditForm';

export const revalidate = 0;

export default async function CredentialDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const cookieStore = await cookies();
  const token = cookieStore.get('azimut_admin_token')?.value;
  const session = token ? verifyAuthToken(token) : null;

  if (!session) {
    redirect('/login');
  }

  const { id } = await params;
  let credential: any = null;
  let error: string | null = null;

  try {
    credential = await prisma.credentials.findUnique({
      where: { id },
    });

    if (!credential) {
      error = 'Credencial não encontrada';
    }
  } catch (err: any) {
    console.error('Credential fetch error:', err);
    error = 'Erro ao carregar credencial. Verifique a conexão com o banco.';
  }

  if (error || !credential) {
    return (
      <div style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}>
        <header
          style={{
            display: 'flex',
            justifyContent: 'space-between',
            alignItems: 'flex-start',
            marginBottom: 24,
          }}
        >
          <div>
            <h1 style={{ margin: '8px 0', fontSize: 32, fontWeight: 700, letterSpacing: '-0.5px' }}>
              Credencial não encontrada
            </h1>
          </div>
        </header>
        <div
          style={{
            padding: '12px 14px',
            borderRadius: 10,
            border: '1px solid rgba(201,35,55,0.35)',
            background: 'rgba(201,35,55,0.12)',
            color: '#fca5a5',
          }}
        >
          {error}
        </div>
      </div>
    );
  }

  return (
    <div style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}>
      <CredentialEditForm credential={credential} />
    </div>
  );
}
