...
export function ProjectsListClient({ projects, error }: ProjectsListClientProps) {
  ...
  const counts = useMemo(() => {
    ...
    const featured = projects.filter((p: any) => p.featured && p.priorityHome >= 1 && p.priorityHome <= 7).length;
  ...
  const filtered = useMemo(() => {
    ...
    if (filterFeatured === 'featured') {
      result = result.filter((p: any) => p.featured && p.priorityHome >= 1 && p.priorityHome <= 7);
  ...
  const homeProjects = filtered.filter((p: any) => p.featured && p.priorityHome >= 1 && p.priorityHome <= 7).sort((a: any, b: any) => (a.priorityHome || 0) - (b.priorityHome || 0));
  const otherProjects = filtered.filter((p: any) => !p.featured || p.priorityHome < 1 || p.priorityHome > 7);
