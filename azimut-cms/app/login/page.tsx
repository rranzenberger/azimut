...
function LoginForm() {
  ...
  async function handleForgotPassword() {
    if (!email.trim()) {
      setForgotMsg('Digite seu email acima antes de solicitar.');
      return;
    }
    setForgotMsg(null);
    ...
    try {
      ...
      if (res.ok) {
        setForgotMsg('Se o email estiver cadastrado, uma nova senha será enviada.');
        setError(null);
      } else {
        setForgotMsg(data.error || 'Erro ao processar. Tente novamente.');
      }
    } catch {
      setForgotMsg('Erro de rede. Verifique sua conexão.');
    } finally {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        const msg = data.error || 'Falha ao autenticar';
        setError(msg);
        setLoading(false);
        return;
      }
    ...
    <form
      ...
      >
        ...
        {!forgotMode ? (
          ...
          <>
            <p style={{ marginTop: 14, marginBottom: 14, fontSize: 13, color: '#94a3b8', lineHeight: 1.5 }}>
              Uma nova senha temporária será gerada e enviada para o email de administração cadastrado.
            </p>
            ...
            {forgotMsg && (
              <p style={{ marginTop: 10, fontSize: 13, color: '#86efac', textAlign: 'center' }}>
                {forgotMsg}
              </p>
            )}
