...
async function testDeepSeekData() {
  ...
  pageViews.forEach((pv: any, i: number) => {
  ...
  interactions.forEach((int: any, i: number) => {
  ...
  if (scores.length === 0) {
    ...
    scores.forEach((score: any, i: number) => {
      ...
      if (score.recommendedProjects) {
        const recommended = JSON.parse(score.recommendedProjects as string);
